...
export function BarrierExplorer() {
  ...
  const resEnergies = useMemo(() =>
    resonanceEnergies(V0, L, 5).filter(er => er <= E_MAX),
    [V0, L]
  )
  ...
  const tvsETraces = useMemo(() => [
    { x: eVals, y: tVals, type: 'scatter', mode: 'lines', name: 'T (transmission)',
      line: { color: DARK.blue, width: 2.5 } },
    { x: eVals, y: rVals, type: 'scatter', mode: 'lines', name: 'R (reflection)',
      line: { color: DARK.red, width: 2 } },
    { x: eVals, y: wkbVals, type: 'scatter', mode: 'lines', name: 'T_WKB',
      line: { color: DARK.orange, width: 1.5, dash: 'dash' }, connectgaps: false },
  ...
  const resonanceShapes = useMemo(() =>
    resEnergies.map(er => ({
      type: 'line', x0: er, x1: er, y0: 0, y1: 1,
  ...
  const resonanceAnnotations = useMemo(() =>
    resEnergies.map((er, i) => ({
      ...
      showarrow: false, font: { size: 9, color: '#666' }, xanchor: 'center',
    })), [resEnergies])
  ...
  const psiLayout = {
    ...
    shapes: [
      { type: 'rect', x0: -half, x1: half, y0: 0, y1: 1,
    ...
    annotations: [
      { x: -half - 0.3, y: 0.05, text: 'incident+reflected', showarrow: false,
  ...
  const potTraces = [
    { x: xPot, y: yPot, type: 'scatter', mode: 'lines', name: 'V(x)',
      line: { color: DARK.orange, width: 2.5 },
      fill: 'tozeroy', fillcolor: V0 >= 0 ? 'rgba(247,127,0,0.12)' : 'rgba(6,214,160,0.12)' },
    { x: [-half - 2, half + 2], y: [E, E], type: 'scatter', mode: 'lines',
      name: `E = ${E.toFixed(2)} a.u.`, line: { color: DARK.blue, width: 1.5, dash: 'dash' } },
    { x: [-half - 2, half + 2], y: [0, 0], type: 'scatter', mode: 'lines', showlegend: false,
  ...
  const potLayout = {
    ...
    xaxis: { ...axis('x (a.u.)') },
    yaxis: { ...axis('V(x), E (a.u.)'), range: [Math.min(V0, 0) * 1.3 - 0.5, Math.max(V0, E) * 1.3 + 0.5] },
  }
    ...
    <>
      ...
      <div style={{ maxWidth: 780 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0 1.5rem' }}>
          <ParameterSlider label="V₀ (barrier height)" value={V0} min={-5} max={10} step={0.1} unit="a.u."
            description="Negative = potential well" onChange={setV0} />
          <ParameterSlider label="L (barrier width)" value={L} min={0.5} max={10} step={0.1} unit="a.u."
            onChange={setL} />
          <ParameterSlider label="E (particle energy)" value={E} min={0.05} max={E_MAX} step={0.05} unit="a.u."
            description="Current operating point" onChange={setE} />
        </div>
...
function helpTitle(topic: ScatteringInfoTopic): string {
  if (topic === 'tvsE')        return 'Barrier — T(E) and R(E)'
  if (topic === 'wavefunction') return 'Barrier — Scattering Wavefunction'
  return 'Barrier — Potential Diagram'
}
