...
`package.json` `version` is auto-generated by the pre-commit hook — never edit it by
hand. Format is `YYYY.MM.DD-N`, where `N` counts commits already made that day on the
current branch, starting at 1. The hook lives at `scripts/hooks/pre-commit` and is
installed into `.git/hooks/` via `scripts/install-hooks.sh`, wired to run automatically
on `npm install` through the `prepare` script.
