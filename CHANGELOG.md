...
- **Missing help ("?") icons** — Stationary States' main title was missing its HelpButton
  ...
  representation (Heisenberg picture) section had no help at all — added a HelpButton and
  new `MatrixInfoPanel.tsx` (matrix elements, parity selection rules, Heisenberg picture);
  the Spin ½ Gates tab had no help anywhere — added a HelpButton and new
  `GatesInfoPanel.tsx` (gates as 2×2 unitaries, Pauli/Hadamard/phase gates, parametric
  rotations)

### Changed
- **Standardized help-button placement** — the "?" now always sits immediately after its
  section title instead of being pushed to the container's right edge. Removed
  `justifyContent: 'space-between'` (`TimeEvolutionExplorer.tsx`, `FourierExplorer.tsx`,
  `FreeParticleExplorer.tsx`) and `flex: 1` on title/toggle styles
  (`HydrogenExplorer.tsx`, `BarrierExplorer.tsx`, `StepExplorer.tsx`, `DeltaExplorer.tsx`,
  `PoschlTellerExplorer.tsx`, `KronigPenneyExplorer.tsx`, `MorseExplorer.tsx`) that had
  caused the two different placements
- **Automatic date-based versioning** — `package.json` `version` is now generated by a new
  pre-commit hook (`scripts/bump-version.mjs`) instead of hand-edited. Format is `YYYY.MM.DD-N`,
  `N` counting commits already made that day; replaces the manual `0.YYYY.MMDD<letter>` scheme.
  Hook is installed via `scripts/install-hooks.sh`, wired to run automatically on `npm install`
  through the new `prepare` script
